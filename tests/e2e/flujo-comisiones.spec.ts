 ...
 * API del admin (cuenta creada con email+contraseña, sin correo), lo usa
 * para el flujo completo y lo elimina al final.
 *
...
test('comisiones: configurar en admin → entregar → deuda → abono', async ({ browser }) => {
	...
	try {
		...
		// ---- 2. Registra el domiciliario dedicado (API con sesión admin): la
		// cuenta se crea con email+contraseña, sin correo de confirmación. ----
		...
		const rReg = await peticion<{
			...
			metodo: 'POST',
			cuerpo: { nombre: nombreDomi, email: emailDomi, password: e.password },
			jar: admin.jar
		});
