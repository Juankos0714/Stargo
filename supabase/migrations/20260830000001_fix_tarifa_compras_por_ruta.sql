-- StarGo · Fix — Las compras con recogida deben cobrar la ruta completa
--
-- La migración de paradas reescribió crear_pedido() y volvió a tratar toda
-- compra_diligencia como una diligencia sin origen: tarifa_base = 0. Esto
-- ignoraba el local de recogida y dejaba un total menor al cotizado. Para todo
-- pedido que incluya barrio de recogida, se resuelve el trayecto origen→destino
-- con la misma matriz que usa un domicilio normal. Las diligencias realmente
-- sin recogida conservan su tarifa automática en cero.
...
BEGIN
    ...
    -- Cualquier pedido con punto de recogida tiene un trayecto cobrable,
    -- incluidos compra_diligencia. Sin origen solo se permiten diligencias
    -- sin recogida y se mantiene tarifa automática en cero.
    ...
    ELSIF v_tipo = 'domicilio' THEN
        -- Un domicilio siempre exige origen y destino válidos.
        RETURN NULL;
    ELSE
        v_tarifa := 0;
    ...
    IF p_recargos IS NOT NULL AND array_length(p_recargos, 1) > 0 THEN
        ...
        FOREACH v_codigo IN ARRAY p_recargos LOOP
            IF v_codigo ~ '^paradas(:[1-9][0-9]*)?$' THEN
                v_paradas := CASE WHEN v_codigo = 'paradas' THEN 1 ELSE split_part(v_codigo, ':', 2)::integer END;
                v_paradas_valor := 3000 * v_paradas;
                v_recargo_total := v_recargo_total + v_paradas_valor;
                v_snapshot := v_snapshot || jsonb_build_object(
                    'codigo', 'paradas',
                    'nombre', format('%s parada%s', v_paradas, CASE WHEN v_paradas = 1 THEN '' ELSE 's' END),
                    'valor', v_paradas_valor
                );
                CONTINUE;
            END IF;
