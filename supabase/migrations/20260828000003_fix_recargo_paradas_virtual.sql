-- ============================================================
-- StarGo · Fix — Persistir el recargo virtual de paradas
-- ============================================================
-- El formulario calcula las paradas a $3.000 por unidad. `paradas` no es una
-- fila fija del catálogo: se envía como `paradas:N` para conservar la cantidad
-- y evitar que crear_pedido() lo rechace al buscar un código inexistente.
-- El código histórico `paradas` sigue siendo válido y equivale a una parada.
...
BEGIN
    ...
    IF p_recargos IS NOT NULL AND array_length(p_recargos, 1) > 0 THEN
        ...
        FOREACH v_codigo IN ARRAY p_recargos LOOP
            -- Código virtual: paradas o paradas:N, donde N es una cantidad positiva.
