...
<div class="min-h-screen bg-gradient-to-b from-slate-50 via-emerald-50/40 to-slate-50">
	<header class="border-b border-slate-200/70 bg-white/80 backdrop-blur">
		<div class="mx-auto flex max-w-3xl items-center justify-between px-6 py-4">
			...
			<nav class="flex items-center gap-3 text-sm">
				<a href="/consultar-estado" class="font-medium text-slate-500 transition hover:text-emerald-600">Consultar estado</a>
			</nav>
		</div>
	</header>
	...
	<main class="mx-auto max-w-3xl px-6 py-12">
		{#if creado}
			<!-- Confirmación -->
			<div class="mx-auto max-w-lg rounded-2xl border border-emerald-200 bg-white p-8 text-center shadow-xl shadow-emerald-500/10">
				...
				<p class="mt-4 inline-block rounded-xl border-2 border-dashed border-emerald-300 bg-emerald-50 px-6 py-3 font-mono text-3xl font-black tracking-widest text-emerald-700">
					{creado.numero}
				</p>
				...
				<div class="mt-6 flex flex-col gap-2.5 sm:flex-row sm:justify-center">
					<a
						...
						class="rounded-xl bg-emerald-500 px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-600"
					>
						Consultar estado
					</a>
					<button
				...
				<form class="mt-8 space-y-6" onsubmit={confirmar}>
					<div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
						<h2 class="mb-4 flex items-center gap-2 text-sm font-bold tracking-wide text-slate-500 uppercase">
							<span class="flex size-5 items-center justify-center rounded-full bg-emerald-500 text-[10px] font-black text-white">1</span>
							Origen
						</h2>
					...
					<div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
						<h2 class="mb-4 flex items-center gap-2 text-sm font-bold tracking-wide text-slate-500 uppercase">
							<span class="flex size-5 items-center justify-center rounded-full bg-emerald-500 text-[10px] font-black text-white">2</span>
							Destino
						</h2>
						<div class="grid gap-4 sm:grid-cols-2">
							<div>
					...
					<div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-5">
						<div class="flex items-center justify-between gap-4">
							<div>
								...
								{:else if calculando}
									<p class="mt-1 flex items-center gap-2 text-sm text-slate-500">
										<span class="size-4 animate-spin rounded-full border-2 border-emerald-500 border-t-transparent"></span>
										Calculando…
									</p>
							...
							{#if calculando}
								<span class="size-8 animate-spin rounded-full border-[3px] border-emerald-500 border-t-transparent"></span>
							{/if}
						</div>
						...
						<button
							...
							class="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-5 py-3.5 text-sm font-bold text-white shadow-lg shadow-emerald-500/25 transition hover:bg-emerald-600 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none"
						>
							{#if confirmando}
