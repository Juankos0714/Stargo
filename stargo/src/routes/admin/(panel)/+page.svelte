...
<div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
	{#each [
		...
		<a
			href={card.href}
			class="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-lg hover:shadow-emerald-500/10"
		>
			<p class="text-sm font-medium text-slate-500">{card.label}</p>
			<p class="mt-2 text-4xl font-extrabold tracking-tight text-slate-900">
				{#if card.valor == null}
					<span class="inline-block size-7 animate-spin rounded-full border-2 border-emerald-500 border-t-transparent align-middle" ></span>
				{:else}
					{card.valor}
				{/if}
			</p>
			<p class="mt-1.5 text-xs text-slate-400 group-hover:text-emerald-600">{card.hint} →</p>
		</a>
	{/each}
</div>
...
<div class="mt-8">
	...
	<div class="grid gap-4 sm:grid-cols-2">
		{#each acciones as accion (accion.href)}
			<a
				href={accion.href}
				class="group flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-5 transition hover:border-emerald-300 hover:shadow-md"
			>
				<div class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600 transition group-hover:bg-emerald-500 group-hover:text-white">
