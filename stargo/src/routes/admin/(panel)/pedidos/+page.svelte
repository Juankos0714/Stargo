					...
					{#each visibles as p (p.id)}
						<tr class="border-b border-slate-100 align-top transition hover:bg-slate-50/60">
							<td class="px-4 py-3">
								...
								<details class="mt-1">
									<summary class="cursor-pointer text-xs font-medium text-emerald-600 hover:underline">
							...
							<td class="px-4 py-3">
								{#if p.domiciliario_nombre}
									<span class="inline-flex items-center gap-1.5 text-sm font-medium text-slate-800">
										<span class="flex size-6 items-center justify-center rounded-full bg-indigo-100 text-[10px] font-bold text-indigo-700">
								...
								{#if p.estado === 'pendiente'}
									<div class="flex flex-col items-end gap-1.5">
										<select
											...
											class="w-44 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-700 transition focus:border-emerald-400 focus:outline-none disabled:opacity-60"
										>
