...
<div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
	...
	{:else}
					...
					{#each origenes as origen (origen.id)}
						<tr>
							...
							{#each destinos as destino (destino.id)}
								...
								<td class="border-b border-slate-100 p-1.5 {esDiagonal ? 'bg-emerald-50/60' : ''}">
									<div class="relative">
										<input
											type="number"
											min="0"
											...
											class="w-full min-w-20 rounded-lg border px-2 py-1.5 text-center text-sm tabular-nums transition {esDiagonal
												? 'border-emerald-200 bg-emerald-50 text-emerald-900 focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/30 focus:outline-none'
												: 'border-slate-200 bg-white text-slate-800 hover:border-emerald-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/30 focus:outline-none'}"
										/>
										{#if guardando[k]}
											<span class="absolute -top-1 -right-1 size-3.5 animate-spin rounded-full border-2 border-emerald-500 border-t-transparent" ></span>
										{:else if guardadoOk[k]}
											<span class="absolute -top-1.5 -right-1 flex size-4 items-center justify-center rounded-full bg-emerald-500 text-white">
		...
		<div class="border-t border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-500">
			<p>
				El <span class="text-emerald-700">placeholder "~$X"</span> indica que el par inverso ya define el precio (matriz simétrica).
