...
{#if cargando && pedidos.length === 0}
	<div class="flex items-center justify-center gap-3 py-20 text-slate-500">
		<span class="size-5 animate-spin rounded-full border-2 border-indigo-500 border-t-transparent"></span>
...
{:else if pestana === 'activos' && activos.length === 0}
	<div class="rounded-2xl border-2 border-dashed border-slate-200 bg-white/60 p-14 text-center">
		<div class="mx-auto flex size-14 items-center justify-center rounded-full bg-indigo-50 text-indigo-400">
			...
			<div class="rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md">
				<div class="flex flex-wrap items-center gap-3 border-b border-slate-100 p-5">
					...
					<div class="w-full sm:w-auto sm:text-right">
						...
						<p class="text-lg font-extrabold text-emerald-700">{formatearPeso(p.tarifa_base)}</p>
					</div>
				</div>
				...
				<div class="flex flex-wrap items-center gap-2 px-5 py-4">
					{#if accion}
						<input
							...
							class="w-full min-w-40 flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 transition placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none"
						/>
						<button
							type="button"
							onclick={() => avanzar(p)}
							disabled={guardando[p.id]}
							class="rounded-lg bg-indigo-500 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-indigo-600 disabled:opacity-60"
						>
					...
					<a
						...
						class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-600 transition hover:border-indigo-300 hover:text-indigo-600"
					>
