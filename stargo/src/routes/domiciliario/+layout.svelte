...
<div class="flex min-h-screen bg-slate-100">
	...
	<aside class="hidden w-60 shrink-0 flex-col bg-slate-950 text-slate-300 md:flex">
		<a href="/" class="flex items-center gap-2.5 px-6 py-5">
			<div class="flex size-8 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-400 to-violet-500">
			...
			<div>
				...
				<div class="text-[10px] tracking-wide text-slate-500 uppercase">Domiciliario</div>
			</div>
		</a>
		...
		<nav class="mt-2 flex-1 space-y-1 px-3">
			<a
				href="/domiciliario"
				class="flex items-center gap-3 rounded-lg bg-indigo-500/15 px-3 py-2.5 text-sm font-medium text-indigo-300"
			>
		...
		<div class="border-t border-white/10 p-4">
			<div class="mb-3 flex items-center gap-3">
				<div class="flex size-9 items-center justify-center rounded-full bg-indigo-500/20 text-sm font-bold text-indigo-300">
					{page.data.email?.charAt(0).toUpperCase()}
				</div>
				<div class="min-w-0">
					<div class="truncate text-xs font-semibold text-white">{page.data.email}</div>
					<div class="text-[10px] text-indigo-400">Repartidor</div>
				</div>
			</div>
			<button
				type="button"
		...
		<header class="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur md:hidden">
			<div class="flex items-center justify-between px-4 py-3">
				<a href="/" class="flex items-center gap-2">
					<div class="flex size-7 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-400 to-violet-500">
					...
					<span class="text-sm font-bold text-slate-900">StarGo Domiciliario</span>
				</a>
				<button
					type="button"
					onclick={salir}
