...
<div class="relative" bind:this={root}>
	<div class="relative">
		...
		<input
			...
			onkeydown={onKeydown}
			class="w-full rounded-xl border border-slate-300 bg-white py-2.5 pr-9 pl-9 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/30 focus:outline-none disabled:cursor-not-allowed disabled:bg-slate-100"
		/>
		{#if seleccionado}
			<button
				type="button"
			...
			{:else}
				{#each filtrados as item, i (item.id)}
					<li
						...
						class="cursor-pointer px-4 py-2 text-sm transition {i === activo ? 'bg-emerald-50 text-emerald-900' : 'text-slate-700 hover:bg-slate-50'}"
						onpointerdown={(e) => {
							e.preventDefault();
							elegir(item);
						}}
