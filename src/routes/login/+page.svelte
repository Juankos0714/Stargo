<script lang="ts">
	...
	import { esCapacitor, storeSession } from '$lib/capacitor-auth';
	...
	async function entrar(e: SubmitEvent) {
		...
		cargando = true;
		error = null;
		const r = await api.post<{ email: string; esAdmin: boolean; esDomiciliario: boolean; access_token?: string; refresh_token?: string }>('/api/login', {
			identificador: email,
			password
		});
		cargando = false;
		...
		// En Capacitor, almacenar tokens en localStorage para que persistan
		// entre requests (las cookies httpOnly no funcionan en CapacitorHttp).
		if (esCapacitor() && r.data?.access_token) {
			storeSession(r.data.access_token, r.data.refresh_token ?? '');
		}
							...
							<label for="password" class="mb-1.5 block text-sm font-medium text-slate-300">Contraseña</label>
								<div class="relative">
									<input
										...
										bind:value={password}
										placeholder="••••••••"
										class="w-full rounded-xl border border-white/10 bg-slate-900/60 min-h-11 px-4 py-2.5 pr-11 text-sm text-white placeholder:text-slate-500 focus:border-primary focus:ring-2 focus:ring-primary/30 focus:outline-none"
									/>
									<button
										type="button"
										onclick={() => (showPassword = !showPassword)}
										tabindex="-1"
										class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-slate-200"
										aria-label={showPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'}
									>
										{#if showPassword}
											<svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
										{:else}
											<svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
										{/if}
									</button>
								</div>
						</div>
